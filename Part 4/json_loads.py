import json
import shelve
from tkinter import filedialog  # todo: keep window on top


def ask_obj_path() -> str:  # todo: change to savename
...
def save_obj(obj_path: str, write_data: tuple, as_num_list, router_num_list) -> None:
    """use 2 lists: 'routers', 'router-neighbors' to name the keys of dict"""
    with shelve.open(obj_path + '/test') as p_obj:
        ...
        for lst in write_data:
            count = 0
            for ele in lst:
                p_obj[key_list[index] + str(num_list[index][count])] = ele
                count += 1
            index += 1
...
if __name__ == '__main__':
    ...
    with shelve.open("../Part 2/test") as test:
        for ele in test:
            print(ele)
